...
async function awardPoints(instructorId, event, token, metadata) {
    try {
...
function RefundAuditRows({ refund, compact }) {
    ...
    return (<>
      ...
      {refund.fromAccountCredit > 0 && (<div className={rowClass}>
      ...
      {refund.createdAt && (<div className={rowClass}>
      ...
      {refund.historyId && (<div className={`${rowClass} gap-3`}>
...
export function EndLessonWizard(props) {
    const { open, onClose, lessonId, pupilId, pupilName, instructorId, durationMinutes, lessonDate, startTime, onCompleted, } = props;
    ...
    const [recordedPaymentMethod, setRecordedPaymentMethod] = useState("cash");
    const [recordedPaymentAmount, setRecordedPaymentAmount] = useState(0);
    ...
    const [finalPaymentLabel, setFinalPaymentLabel] = useState("Skipped");
    const [pricingRules, setPricingRules] = useState([]);
    const [pupilPostcode, setPupilPostcode] = useState(undefined);
    ...
    useEffect(() => {
        ...
        const onRefund = (e) => {
            ...
            if (!detail)
                return;
            if (detail.pupilId && detail.pupilId !== pupilId)
                return;
            setRefundResults((prev) => [...prev, detail]);
        };
        window.addEventListener("dsm-refund-recorded", onRefund);
        return () => window.removeEventListener("dsm-refund-recorded", onRefund);
    }, [open, pupilId]);
    const sortedRefunds = useMemo(() => [...refundResults].sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()), [refundResults]);
    ...
    useEffect(() => {
        ...
        setProgressComments(typeof draft?.progressComments === "string" ? draft.progressComments : "");
    ...
    useEffect(() => {
        if (!open || done)
            return;
        try {
            sessionStorage.setItem(draftKey, JSON.stringify({ step, notes, amount, paymentMethod, levels, progressComments }));
        }
        catch {
            /* ignore */
        }
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (cancelled)
                return;
            if (r) {
                const overspeed = r.overspeed_count ??
                    (Array.isArray(r.overspeed_events)
                        ? r.overspeed_events.length
                        : 0);
                setRoute({
                ...
                setFinalDistance(r.distance_miles);
            }
            else {
                setRoute(null);
            }
            const { data: prof } = await supabase
                ...
                .maybeSingle();
            if (!cancelled && prof && prof.hourly_rate != null) {
                setHourlyRate(Number(prof.hourly_rate));
            }
            ...
            if (!cancelled && p) {
                ...
                setBalance(-Number(p.balance_owed ?? 0));
                const pc = p.postcode;
                if (pc)
                    setPupilPostcode(pc);
            }
            ...
            if (!cancelled && Array.isArray(rulesData)) {
                setPricingRules(rulesData.map((r) => ({
                    id: String(r.id),
    ...
    const handleRecordPayment = async () => {
        ...
        try {
            ...
            setRecordedPaymentMethod(paymentMethod === "waived"
                ? "waived"
                : paymentMethod === "already_paid"
                    ? "already_paid"
                    : paymentMethod);
            setRecordedPaymentAmount(amt);
            setFinalPaymentLabel(paymentMethod === "cash"
                ? "Paid · Cash"
                : paymentMethod === "bank"
                    ? "Paid · Bank transfer"
            ...
            toast.error(e?.message ?? "Couldn't record payment");
        }
        finally {
            setPaymentSaving(false);
        }
    ...
    useEffect(() => {
        if (!qrPaymentId || paymentRecorded)
            return;
            ...
            try {
                ...
                if (status === "succeeded" || status === "completed" || status === "paid") {
                    ...
                    const { data: lRowQr } = await supabase
                        ...
                        .eq("id", lessonId)
                        .maybeSingle();
                    const dueQr = Number(lRowQr?.amount_due ?? lessonCost);
                    await supabase
                        .from("lessons")
                        .update({
                        payment_status: "paid",
    ...
    const completeEol = async () => {
        ...
        const updatedEntries = Object.entries(levels).filter(([, v]) => v && v !== "not_started");
        ...
        if (updatedEntries.length > 0) {
            try {
                ...
                const perLabelTargets = updatedEntries.map(([label, status]) => {
                    ...
                    const keys = mapped && mapped.length > 0 ? mapped : [`eol_${slugify(label)}`];
                    return { label, status, keys };
                });
                // Read existing statuses for all referenced keys.
                const itemKeys = Array.from(new Set(perLabelTargets.flatMap((t) => t.keys)));
                const existing = {};
                ...
                for (const r of (existingRows ?? [])) {
                    const s = r.status;
                    if (s && s in LEVEL_RANK)
                        existing[r.item_key] = s;
                }
                ...
                for (const { label, status, keys } of perLabelTargets) {
                    ...
                    for (const k of keys) {
                        ...
                        if (newRank >= priorRank) {
                            ...
                            if (priorRank > maxBlockingExisting)
                                maxBlockingExisting = priorRank;
                        }
                    }
                    if (anyApplied) {
        ...
        try {
            const { data: p } = await supabase
                ...
                .maybeSingle();
            if (p) {
                const total = Number(p.total_hours_completed ?? 0);
                const target = Number(p.course_total_hours ?? 0);
    ...
    const ProgressBar = ({ current }) => (<div className="flex items-center gap-2 mb-4">
      ...
      <span className="text-[11px] ml-2" style={{ ...POPPINS, color: "#6B7280", fontWeight: 600 }}>
        Step {current} of 3
      </span>
    </div>);
    const footer = (() => {
        if (done) {
            return (<div>
          <button type="button" onClick={finish} className="w-full h-11 rounded-lg text-[14px] font-semibold text-white" style={{ backgroundColor: "#1877D6", border: "none" }}>
            Done
          </button>
        </div>);
        }
        if (step === 1) {
            return (<div>
          <button type="button" disabled={!notes.trim()} onClick={() => setStep(2)} className="w-full h-11 rounded-lg text-[14px] font-semibold text-white" style={{
        ...
        if (step === 2) {
          ...
          <button type="button" onClick={() => setStep(1)} disabled={paymentSaving} className="flex-1 h-11 rounded-lg text-[14px] font-semibold" style={{
          ...
          <button type="button" onClick={skipPayment} disabled={paymentSaving} className="flex-1 h-11 rounded-lg text-[14px] font-semibold text-white" style={{
        ...
        return (<div className="flex gap-2">
        <button type="button" onClick={() => setStep(2)} disabled={completing} className="flex-1 h-11 rounded-lg text-[14px] font-semibold" style={{
                backgroundColor: "#F8F9FB",
                color: "#0B1F3A",
                border: "1px solid #EEF2F7",
        ...
        <button type="button" onClick={completeEol} disabled={completing} className="flex-1 h-11 rounded-lg text-[14px] font-semibold text-white flex items-center justify-center gap-2" style={{
                backgroundColor: "#1877D6",
                border: "none",
                opacity: completing ? 0.7 : 1,
            }}>
    ...
    return (<>
      <BottomSheet title="End of lesson" subtitle={pupilName ?? undefined} onClose={() => !completing && !paymentSaving && onClose()} footer={footer}>
      <div style={{ ...POPPINS, paddingBottom: 0 }}>
        ...
        {!done && step === 1 && (<div>
            ...
            <label className="block text-[12px] mt-4 mb-1" style={{ color: "#0B1F3A", fontWeight: 600 }}>
              Lesson notes
            </label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="What did you cover? Any observations?" rows={4} className="w-full p-3 text-[14px]" style={{
                borderRadius: 10,
            ...
            {route && (<div className="mt-4 p-3" style={{
                    borderRadius: 10,
                    backgroundColor: "#F8F9FB",
                    border: "0.5px solid #EEF2F7",
                }}>
                <div className="text-[11px] uppercase mb-2" style={{ color: "#6B7280", letterSpacing: "0.05em", fontWeight: 600 }}>
                  Route summary
                </div>
        ...
        {!done && step === 2 && (<div>
            ...
            <div className="mt-3 p-3 flex items-center justify-between" style={{
                borderRadius: 10,
                backgroundColor: "#F8F9FB",
                border: "0.5px solid #EEF2F7",
            }}>
              <div>
                ...
                {pricing.adjustments.length > 0 && (<div className="mt-1" style={{ fontSize: 11, color: "#6B7280", lineHeight: 1.4 }}>
                    <div>Base: £{baseCost.toFixed(2)}</div>
                    {pricing.adjustments.map((a, i) => (<div key={i} style={{ color: "#0B1F3A" }}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              {[
                ...
                return (<button key={m.k} type="button" onClick={() => setPaymentMethod(m.k)} className="flex flex-col items-center justify-center gap-1 p-3" style={{
                        borderRadius: 10,
            ...
            <input type="number" inputMode="decimal" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full p-3 text-[14px]" style={{
                borderRadius: 10,
                border: "0.5px solid #EEF2F7",
                ...POPPINS,
                color: "#0B1F3A",
            ...
            <button type="button" onClick={handleRecordPayment} disabled={paymentSaving} className="mt-5 w-full h-11 rounded-lg text-[14px] font-semibold text-white flex items-center justify-center gap-2" style={{
                backgroundColor: "#1877D6",
                border: "none",
                opacity: paymentSaving ? 0.7 : 1,
            }}>
            ...
            <button type="button" onClick={generateQrPayment} disabled={qrGenerating || paymentSaving || !!qrUrl} className="mt-2 w-full h-11 rounded-lg text-[14px] font-semibold flex items-center justify-center gap-2" style={{
                backgroundColor: "#0B1F3A",
                color: "#fff",
                border: "none",
                opacity: qrGenerating || !!qrUrl ? 0.7 : 1,
            }}>
              {qrGenerating ? (<IconLoader2 stroke={1.5} size={16} className="animate-spin"/>) : (<QrCode size={16}/>)}
              {qrGenerating ? "Generating…" : qrUrl ? "QR code ready" : "Show QR code"}
            </button>

            {qrUrl && (<div className="mt-3 p-4 flex flex-col items-center" style={{
                    borderRadius: 10,
                    backgroundColor: "#F8F9FB",
                    border: "0.5px solid #EEF2F7",
                }}>
        ...
        {!done && step === 3 && (<div>
            ...
            {sortedRefunds.length > 0 && (<div className="mt-3 p-3" style={{ borderRadius: 10, backgroundColor: "#FEF2F2", border: "0.5px solid #FBD5D5" }}>
                <div className="text-[13px] mb-2" style={{ color: "#CC2229", fontWeight: 700 }}>
                  Refund activity
                </div>
                {sortedRefunds.map((refund, idx) => (<div key={refund.historyId ?? `${refund.createdAt ?? idx}-${idx}`} className={idx > 0 ? "mt-2 pt-2 border-t border-[#FBD5D5]" : undefined}>
                    <RefundAuditRows refund={refund}/>
                  </div>))}
              </div>)}

            {paymentRecorded && lastPaymentResult && (<div className="mt-3 p-3" style={{ borderRadius: 10, backgroundColor: "#F8F9FB", border: "0.5px solid #EEF2F7" }}>
                ...
                {lastPaymentResult.overpayment > 0 && (<div className="flex justify-between text-[12px] py-1">
                ...
                {lastPaymentResult.lessonsLeftPartial > 0 && (<div className="flex justify-between text-[12px] py-1">
                ...
                <button type="button" onClick={() => setReceiptOpen(true)} className="mt-2 w-full flex items-center justify-center gap-2" style={{
                    padding: "8px 0",
                    borderRadius: 8,
                    backgroundColor: "#fff",
                    border: "0.5px solid #EEF2F7",
            ...
            <div className="mt-3 flex flex-col gap-2">
              {COMPETENCIES.map((c) => {
                ...
                return (<div key={c} className="p-2" style={{
                        borderRadius: 10,
                        backgroundColor: "#F8F9FB",
                        border: "0.5px solid #EEF2F7",
                    }}>
                    <div className="text-[13px] mb-2" style={{ color: "#0B1F3A", fontWeight: 600 }}>
                      {c}
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {LEVELS.map((l) => {
                        const active = current === l.key;
                        return (<button key={l.key} type="button" aria-label={`${c}: ${l.label}`} title={l.label} onClick={() => setLevels((prev) => ({ ...prev, [c]: l.key }))} style={{
                                width: 30,
                                height: 30,
                                borderRadius: 999,
                                display: "flex",
                      ...
                      {current !== "not_started" && (<button type="button" onClick={() => setLevels((prev) => {
                            const next = { ...prev };
                            delete next[c];
                            return next;
                        })} className="text-[11px] ml-1" style={{
                            color: "#6B7280",
            ...
            <div className="mt-3 p-2 text-[11px] flex flex-wrap gap-x-3 gap-y-1" style={{ color: "#6B7280" }}>
              {LEVELS.map((l) => (<span key={l.key} className="flex items-center gap-1">
                  <span style={{
                    width: 10,
                    height: 10,
                    borderRadius: 999,
            ...
            <textarea value={progressComments} onChange={(e) => setProgressComments(e.target.value)} rows={3} className="w-full p-3 text-[14px]" style={{
                borderRadius: 10,
                border: "0.5px solid #EEF2F7",
                ...POPPINS,
                color: "#0B1F3A",
        ...
        {/* COMPLETION SCREEN */}
        {done && (<div className="flex flex-col items-center text-center py-4">
            {courseComplete ? (<>
                <div style={{ position: "relative" }}>
                  <PartyPopper size={64} color="#1877D6"/>
                  <div className="confetti-burst" aria-hidden/>
                </div>
                <div className="text-[20px] mt-3" style={{ color: "#0B1F3A", fontWeight: 700 }}>
                  Course complete! 🎓
                </div>
            ...
            <div className="w-full mt-4 p-3" style={{
                borderRadius: 10,
                backgroundColor: "#F8F9FB",
                border: "0.5px solid #EEF2F7",
                textAlign: "left",
              ...
              {finalDistance != null && (<div className="flex justify-between text-[13px] py-1">
              ...
              {sortedRefunds.length > 0 && (<div className="mt-2 pt-2" style={{ borderTop: "1px solid #EEF2F7" }}>
                  <div className="text-[12px] mb-1" style={{ color: "#CC2229", fontWeight: 700 }}>
                    Refund activity
                  </div>
                  {sortedRefunds.map((refund, idx) => (<div key={refund.historyId ?? `${refund.createdAt ?? idx}-${idx}`} className={idx > 0 ? "mt-2 pt-2" : undefined} style={idx > 0 ? { borderTop: "1px solid #EEF2F7" } : undefined}>
                      <RefundAuditRows refund={refund} compact/>
                    </div>))}
                </div>)}

              {paymentRecorded && lastPaymentResult && (<div className="mt-2 pt-2" style={{ borderTop: "1px solid #EEF2F7" }}>
                  ...
                  {lastPaymentResult.overpayment > 0 && (<div className="flex justify-between text-[12px] py-0.5">
                      <span style={{ color: "#6B7280" }}>Overpayment</span>
                      <span style={{ color: "#0B1F3A", fontWeight: 600 }}>£{lastPaymentResult.overpayment.toFixed(2)}</span>
                    </div>)}
                  <div className="flex justify-between text-[12px] py-0.5">
                  ...
                  {lastPaymentResult.lessonsLeftPartial > 0 && (<div className="flex justify-between text-[12px] py-0.5">
                      <span style={{ color: "#6B7280" }}>Left partially paid</span>
                      <span style={{ color: "#0B1F3A", fontWeight: 600 }}>{lastPaymentResult.lessonsLeftPartial}</span>
                    </div>)}
                  <button type="button" onClick={() => setReceiptOpen(true)} className="mt-2 w-full flex items-center justify-center gap-2" style={{
    ...
    {receiptOpen && lastPaymentResult && (<PaymentReceipt pupilName={pupilName} lessonDate={lessonDate} startTime={startTime} durationMinutes={durationMinutes} lessonCost={lessonCost} amountPaid={recordedPaymentAmount} method={recordedPaymentMethod} result={lastPaymentResult} onClose={() => setReceiptOpen(false)}/>)}
  </>);
}
