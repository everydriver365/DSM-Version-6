...
function DeliveryIndicator({
  ...
  return message.read_at ? (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
      ...
      <span>{timeLabel(message.read_at)}</span>
    </span>
  ) : (
...
function InstructorDMThread() {
  ...
  const senderNameRef = useRef("Instructor");

  useEffect(() => {
    senderNameRef.current = firstName(otherInstructor?.name) || "Instructor";
  }, [otherInstructor]);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (row.from_instructor_id !== userId && !atBottomRef.current) {
