...
function HomePage() {
  ...
  return (
    <div className="pb-safe" style={{ minHeight:'100dvh', width:'100%', maxWidth:'100vw', background:'#F7FAFC', paddingTop:'calc(60px + env(safe-area-inset-top, 0px))', paddingBottom:'calc(80px + env(safe-area-inset-bottom, 0px))', fontFamily:'Poppins, sans-serif', overflowX:'hidden' }}>
      {notifBanner}
      ...
      {/* ============ NEW: SINGLE-SCROLL BODY ============ */}
      <div style={{ background:'#F7FAFC', fontFamily:'Poppins, sans-serif' }}>
        {/* SECTION 1 — NAVY GREETING HEADER */}
        {(() => {
          const now = new Date();
          const h = now.getHours();
          const timeEmoji = h < 12 ? '☀️' : h < 18 ? '🌤️' : '🌙';
          ...
          return (
            <div style={{ background:'linear-gradient(160deg, #0B1F3A 0%, #1A3A6B 100%)', padding:'16px 16px 64px', position:'relative', overflow:'hidden' }}>
              <div aria-hidden style={{ position:'absolute', right:-60, top:-60, width:200, height:200, borderRadius:'50%', background:'#FFFFFF', opacity:0.05 }} />
              <div aria-hidden style={{ position:'absolute', left:-30, bottom:20, width:120, height:120, borderRadius:'50%', background:'#00B5A5', opacity:0.05 }} />
              <div style={{ position:'relative', zIndex:2 }}>
                <div style={{ fontSize:11, color:'rgba(255,255,255,0.5)', textTransform:'uppercase', letterSpacing:'0.12em', fontWeight:600 }}>{timeEmoji} Good {timeLabel}</div>
                <div style={{ fontSize:28, fontWeight:900, color:'#FFFFFF', lineHeight:1, marginTop:4 }}>{firstName}</div>
        ...
        {/* SECTION 2 — NEXT LESSON HERO (lifted from Today panel, sits below navy header) */}
        <div style={{ margin:'-44px 16px 0', position:'relative', zIndex:3 }}>
        {/* NEXT LESSON HERO */}
      ...
      {(() => {
        ...
        {/* ============ NEW: THIS WEEK ============ */}
        <div style={{ background:'#FFFFFF', borderRadius:20, margin:'12px 16px 0', boxShadow:'0 2px 12px rgba(15,32,68,0.06)', overflow:'hidden', fontFamily:'Poppins, sans-serif' }}>
          <div style={{ padding:'16px 20px', borderBottom:'0.5px solid #F3F4F6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontSize:14, fontWeight:600, color:'#0F2044' }}>This week</div>
            <button type="button" onClick={() => navigate({ to:'/schedule' })} style={{ background:'none', border:'none', padding:0, cursor:'pointer', fontSize:12, color:'#1A52A0' }}>Schedule →</button>
          </div>
          <div style={{ display:'flex', gap:8, padding:'12px 16px', borderBottom:'0.5px solid #F3F4F6' }}>
            <div style={{ flex:1, background:'#F7FAFC', border:'0.5px solid #E2E6ED', borderRadius:10, padding:'10px 14px', textAlign:'center' }}>
              <div style={{ fontSize:16, fontWeight:700, color:'#0F2044' }}>{weekLessonsTotal}</div>
              <div style={{ fontSize:11, color:'#6B7280', marginTop:2 }}>lessons</div>
            </div>
            <div style={{ flex:1, background:'#F7FAFC', border:'0.5px solid #E2E6ED', borderRadius:10, padding:'10px 14px', textAlign:'center' }}>
              <div style={{ fontSize:16, fontWeight:700, color:'#16A34A' }}>£{Math.round(weekEarnings)}</div>
              <div style={{ fontSize:11, color:'#6B7280', marginTop:2 }}>earned</div>
            </div>
            <div style={{ flex:1, background:'#F7FAFC', border:'0.5px solid #E2E6ED', borderRadius:10, padding:'10px 14px', textAlign:'center' }}>
              <div style={{ fontSize:16, fontWeight:700, color:'#7C3AED' }}>{upcomingTests?.length || 0}</div>
              <div style={{ fontSize:11, color:'#6B7280', marginTop:2 }}>tests</div>
            </div>
          </div>
          <div style={{ padding:'12px 16px' }}>
            <div style={{ fontSize:11, color:'#9CA3AF', fontWeight:600, marginBottom:8 }}>Tomorrow</div>
            {tomorrowLessons.length === 0 ? (
              <div style={{ fontSize:12, color:'#9CA3AF' }}>No lessons tomorrow</div>
            ) : (
              tomorrowLessons.slice(0, 4).map((l: any, idx: number) => {
                const nm = l.pupils?.first_name || l.pupils?.name || 'Pupil';
                const initials = String(nm).split(/\s+/).map((s:string) => s.charAt(0)).slice(0,2).join('').toUpperCase();
                const bg = l.pupils?.calendar_colour || '#1A52A0';
                return (
                  <div key={l.id} onClick={() => navigate({ to:'/lessons/$id', params:{ id: l.id } })} style={{ display:'flex', alignItems:'center', gap:10, padding:'6px 0', borderTop: idx === 0 ? 'none' : '0.5px solid #F3F4F6', cursor:'pointer' }}>
                    <div style={{ width:28, height:28, borderRadius:'50%', background:bg, color:'#fff', fontSize:11, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>{initials}</div>
                    <div style={{ flex:1, minWidth:0, fontSize:13, color:'#0F2044' }}>{String(l.lesson_time || '').slice(0,5)} · {nm}</div>
                    <div style={{ fontSize:11, color:'#9CA3AF' }}>{l.duration_minutes || 60}m</div>
                  </div>
                );
              })
            )}
        ...
        <div style={{ background:'#FFFFFF', borderRadius:20, margin:'12px 16px 0', boxShadow:'0 2px 12px rgba(15,32,68,0.06)', overflow:'hidden', fontFamily:'Poppins, sans-serif' }}>
          <div style={{ padding:'16px 20px', borderBottom:'0.5px solid #F3F4F6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontSize:14, fontWeight:600, color:'#0F2044' }}>Money</div>
            <button type="button" onClick={() => navigate({ to:'/payments' })} style={{ background:'none', border:'none', padding:0, cursor:'pointer', fontSize:12, color:'#1A52A0' }}>View all →</button>
          </div>
          <div style={{ padding:'14px 16px', borderBottom:'0.5px solid #F3F4F6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <div style={{ fontSize:18, fontWeight:700, color:'#0F2044' }}>£{Math.round(weekEarnings)}</div>
          </div>
          {outstanding > 0 && (
            <div onClick={() => setOutstandingOpen(true)} style={{ padding:'14px 16px', borderBottom:'0.5px solid #F3F4F6', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer' }}>
              <div style={{ fontSize:13, color:'#6B7280' }}>Outstanding</div>
              <div style={{ fontSize:18, fontWeight:700, color:'#CC2229' }}>£{Math.round(outstanding)}</div>
            </div>
          )}
          <div style={{ padding:'14px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <div style={{ fontSize:13, color:'#6B7280' }}>Today</div>
            <div style={{ fontSize:18, fontWeight:700, color:'#16A34A' }}>£{Math.round(todayEarnings)}</div>
          </div>
        </div>
