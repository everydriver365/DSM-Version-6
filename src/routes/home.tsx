...
<section
      ...
      {moreMenuOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 200, display: 'flex', flexDirection: 'column' }}>
          <div onClick={() => setMoreMenuOpen(false)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#F7FAFC', borderRadius: '20px 20px 0 0', maxHeight: '85vh', overflowY: 'auto', paddingBottom: 'calc(80px + env(safe-area-inset-bottom, 0px))' }}>
            ...
            <div style={{ padding: '8px 20px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setMoreMenuOpen(false)} style={{ background: '#F3F4F6', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div key={group.label} style={{ padding: '0 16px 16px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                  {group.items.map(item => (
                    <button
                      key={item.label}
                      onClick={() => { setMoreMenuOpen(false); navigate({ to: item.route as never }); }}
            ...
            <div style={{ padding: '0 16px 16px', display: 'flex', gap: 8 }}>
              <button
                onClick={() => { setMoreMenuOpen(false); navigate({ to: '/dsm-live' as never }); }}
              ...
              <button
                onClick={() => { setMoreMenuOpen(false); navigate({ to: '/marketplace' as never }); }}
