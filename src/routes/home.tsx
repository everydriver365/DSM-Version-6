  ...
  return (
    <div className="min-h-screen pb-24 pb-safe" style={{ ...POPPINS, backgroundColor: '#F2F4F8', paddingTop: 'calc(60px + env(safe-area-inset-top, 0px))' }}>
      ...
      >
        ...
        {tab === "next" ? (
          (() => {
            const l = nextTabLessons[0] ?? nextLesson;
            if (!l) {
              return (
                <div
                  style={{
            ...
            const lDate = lessonDateTime(l);
            const lEndMs = lDate.getTime() + (l.duration_minutes ?? 60) * 60000;
            const isPast = lEndMs < now.getTime();
            const isCurrent = lDate.getTime() <= now.getTime() && now.getTime() <= lEndMs;
            const pName = l.pupils?.name ?? "Pupil";
            ...
            const balance = l.pupils?.balance_owed ?? 0;
            const pickup = l.pickup_location ?? l.pupils?.postcode ?? "";
            const amountDue = typeof l.amount_due === "number" ? l.amount_due : 0;
            const unpaidPast = l.payment_status === "unpaid" && isPast && amountDue > 0;
            const mapsHref = pickup
              ? `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(pickup)}`
              : null;
            const smsHref = phone ? `sms:${phone}?&body=${encodeURIComponent("On my way!")}` : null;
            const reminderHref = phone
              ? `sms:${phone}?&body=${encodeURIComponent(`Hi ${pName.split(" ")[0]}, just a reminder that £${amountDue.toFixed(2)} is outstanding from your last lesson. Thanks!`)}`
              : null;
            ...
            const tile = (
              ...
              >
                {icon}
                <span style={{ fontSize: 11, fontWeight: 600, color: "#1A1A2E" }}>{label}</span>
              </button>
            );
            ...
            return (
              ...
              >
                {/* Header */}
                <div style={{ background: "#0F2044", padding: "12px 16px", color: "#FFFFFF" }}>
                  <div
                    style={{
                      fontSize: 10,
                      textTransform: "uppercase",
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, marginTop: 2 }}>
                    {formatTime(l)} · {formatDayLabel(lDate)}
                  </div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", marginTop: 2 }}>
                    {formatDuration(l.duration_minutes ?? 60)}
                  </div>
                </div>
                ...
                >
                  ...
                  >
                    {balance > 0
                      ...
                      : balance < 0
                        ? `£${Math.abs(balance).toFixed(2)} credit`
                        : "£0.00"}
                  </div>
                </div>
                ...
                >
                  {tile(
                    <Navigation size={20} color="#1A52A0" />,
                    "Navigate",
                    () => {
                      if (mapsHref) window.open(mapsHref, "_blank");
                      else toast("No pickup address set");
                    },
                    !mapsHref,
                  )}
                  {tile(
                    <Phone size={20} color="#CC2229" />,
                    "Call",
                    () => {
                  ...
                  {tile(
                    <MessageSquare size={20} color="#1A52A0" />,
                    "Text",
                    () => {
                ...
                {unpaidPast && (
                  ...
                  >
                    <PoundSterling size={16} color="#B45309" />
                    <div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: "#92400E" }}>
                      Payment outstanding: £{amountDue.toFixed(2)}
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        if (reminderHref) window.location.href = reminderHref;
                        else toast("No phone number");
                      }}
                      style={{
                        background: "#B45309",
