        ...
        >
          ...
          >
            ...
            <div style={{ padding: "0 16px" }}>
              ...
              <input
                value={editingPartner.video_embed_url ?? editingPartner.video_url ?? ""}
                onChange={(e) => patch({ video_embed_url: e.target.value, video_url: null })}
                placeholder="https://youtube.com/... or https://vimeo.com/..."
                style={partnerInputStyle}
              />
              {getVideoThumbnail(editingPartner.video_embed_url ?? editingPartner.video_url ?? "") ? (
                <img
                  src={getVideoThumbnail(editingPartner.video_embed_url ?? editingPartner.video_url ?? "") ?? undefined}
                  alt="Video preview"
                  style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 8, marginBottom: 12 }}
                />
              ) : null}
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  const path = `benefits/partners/${editingPartner.id}.mp4`;
                  const { error } = await supabase.storage
                  ...
                  if (!error) {
                    const { data: { publicUrl } } = supabase.storage
                      .from("marketplace-videos")
                      .getPublicUrl(path);
                    patch({ video_url: publicUrl, video_embed_url: null });
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "0 16px" }}>
              ...
              <input
                value={editingPerk.video_embed_url ?? editingPerk.video_url ?? ""}
                onChange={(e) => patchPerk({ video_embed_url: e.target.value, video_url: null })}
                placeholder="https://youtube.com/... or https://vimeo.com/..."
                style={partnerInputStyle}
              />
              {getVideoThumbnail(editingPerk.video_embed_url ?? editingPerk.video_url ?? "") ? (
                <img
                  src={getVideoThumbnail(editingPerk.video_embed_url ?? editingPerk.video_url ?? "") ?? undefined}
                  alt="Video preview"
                  style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 8, marginBottom: 12 }}
                />
              ) : null}
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  const path = `benefits/perks/${editingPerk.id}.mp4`;
                  const { error } = await supabase.storage
                    .from("marketplace-videos")
                    .upload(path, file, { upsert: true, contentType: file.type });
                  if (!error) {
                    const { data: { publicUrl } } = supabase.storage
                      .from("marketplace-videos")
                      .getPublicUrl(path);
                    patchPerk({ video_url: publicUrl, video_embed_url: null });
                    toast.success("Video uploaded");
                  } else {
                    toast.error("Upload failed");
                  }
