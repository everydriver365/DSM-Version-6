...
const THEMES: Record<Theme, { bg: string; primary: string; accent: string; surface: string; muted: string; border: string; isDark: boolean }> = {
  classic: { bg: "#FFFFFF", primary: "#0F2044", accent: "#1A52A0", surface: "#F8FAFC", muted: "#475569", border: "#E2E8F0", isDark: false },
...
function InstructorMiniSite() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#FFFFFF", color: "#0F2044", fontFamily: "system-ui" }}>
        Loading…
      </div>
    );
  }
  ...
  if (!instructor) {
    return (
      <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#FFFFFF", color: "#0F2044", fontFamily: "system-ui", padding: 24, textAlign: "center" }}>
        <div>
          ...
          <Link to="/courses" style={{ color: "#1A52A0", fontWeight: 600, textDecoration: "underline" }}>
            Browse all courses →
          </Link>
        </div>
      </div>
