...
const FALLBACK_PERKS: Perk[] = [
  ...
  },
  {
    ...
    provider: 'HMCA',
    category: 'Health',
    description: 'Private health insurance exclusively for EDP members.',
    saving: 'EDP exclusive rates',
    minTier: 'website',
    logo: '❤️',
  },
  {
  ...
  },
  {
    ...
    provider: 'DIA',
    category: 'Finance',
    description: 'Full DIA membership included free with your EDP subscription.',
  ...
  useEffect(() => {
    (async () => {
      ...
      const allPerks: Perk[] = (data as any[]).map((row) => ({
        ...
        dbId: row.id,
        name: row.name,
        provider: row.partner?.name ?? 'EDP partner',
