...
function EndOfDayPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#FFFFFF", fontFamily: "Inter, sans-serif", paddingBottom: 80 }}>
      {/* Top bar */}
      <div style={{ backgroundColor: "#0C2340", color: "#FFFFFF", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ padding: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        ...
        <Stat label="Outstanding" value={`£${stats.outstanding.toFixed(0)}`} color={stats.outstanding > 0 ? "#DC2626" : "#0C2340"} />
      </div>
        ...
        <Card>
          ...
          {outstandingEols.map((l) => (
            <Row key={`eol-${l.id}`}>
              ...
              <SmallBtn color="#1A4A6E" onClick={() => setEolLesson(l)}>Complete EOL</SmallBtn>
            </Row>
          ))}
          {unpaidLessons.map((l) => (
            <Row key={`pay-${l.id}`}>
      ...
      <Card>
        ...
        {lessons.length === 0 ? (
          ...
          lessons.map((l) => (
            ...
            >
              <span style={{ fontSize: 12, fontWeight: 600, color: "#0C2340", minWidth: 50 }}>
                {l.lesson_time?.slice(0, 5) ?? "--:--"}
              </span>
              <span style={{ fontSize: 13, color: "#0C2340", flex: 1 }}>
      ...
      {/* Section 4: Tomorrow */}
      <Card>
        <Heading icon={<CalendarIcon size={16} color="#1A4A6E" />} title="Tomorrow" />
        {tomorrowLessons.length === 0 ? (
          <div style={{ fontSize: 13, color: "#6B7280" }}>No lessons booked for tomorrow.</div>
        ) : (
          <>
            <div style={{ fontSize: 13, color: "#0C2340", marginBottom: 8 }}>
            ...
            {tomorrowLessons.slice(0, 3).map((l) => (
              <div key={l.id} style={{ display: "flex", gap: 10, padding: "6px 0", fontSize: 13, color: "#0C2340" }}>
      ...
      <Card>
        ...
        <div style={{ position: "relative" }}>
          ...
          >
            {listening ? <MicOff size={16} color="#FFFFFF" /> : <Mic size={16} color="#0C2340" />}
          </button>
        </div>
...
function Heading({ icon, title }: { icon?: React.ReactNode; title: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
      {icon}
      <div style={{ fontSize: 14, fontWeight: 700, color: "#0C2340" }}>{title}</div>
    </div>
  );
}
...
function Stat({ label, value, color, hint }: { label: string; value: string; color?: string; hint?: string }) {
  return (
    ...
    >
      <div style={{ fontSize: 20, fontWeight: 700, color: color ?? "#0C2340" }}>
        {value}
