export function applyPricingRules(baseRate, rules, context) {
    const adjustments = [];
    let total = baseRate;
    ...
    for (const rule of rules.filter(r => r.is_active)) {
        ...
        if (rule.rule_type === 'day_of_week' && c.days) {
            ...
            applies = c.days.some((d) => d.toLowerCase() === dayNames[dayOfWeek].toLowerCase() ||
        ...
        if (rule.rule_type === 'postcode_zone' && c.postcodes && context.postcode) {
            ...
            applies = c.postcodes.some((p) => context.postcode.toUpperCase().startsWith(p.trim().toUpperCase()) ||
